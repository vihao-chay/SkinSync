...
public class IngredientConflictService : IIngredientConflictService
{
    ...
    public async Task<IReadOnlyCollection<IngredientConflictRuleDto>> GetRulesAsync(CancellationToken cancellationToken)
    {
        return await _dbContext.IngredientConflictRules
            .AsNoTracking()
            .Include(x => x.PrimaryIngredientEntity)
            .Include(x => x.ConflictingIngredientEntity)
            .OrderBy(x => x.PrimaryIngredientEntity != null ? x.PrimaryIngredientEntity.Name : x.PrimaryIngredient)
            .ThenBy(x => x.ConflictingIngredientEntity != null ? x.ConflictingIngredientEntity.Name : x.ConflictingIngredient)
            .Select(x => new IngredientConflictRuleDto
            {
                Id = x.Id,
                PrimaryIngredient = x.PrimaryIngredientEntity != null ? x.PrimaryIngredientEntity.Name : x.PrimaryIngredient ?? string.Empty,
                ConflictingIngredient = x.ConflictingIngredientEntity != null ? x.ConflictingIngredientEntity.Name : x.ConflictingIngredient ?? string.Empty,
                Severity = x.Severity,
                Message = x.Message,
        ...
        HashSet<string> seen)
    {
        ...
        {
            var primaryIngredient = rule.PrimaryIngredientEntity?.Name ?? rule.PrimaryIngredient;
            var conflictingIngredient = rule.ConflictingIngredientEntity?.Name ?? rule.ConflictingIngredient;
            if (!ContainsIngredient(productA.Ingredient, primaryIngredient) ||
                !ContainsIngredient(productB.Ingredient, conflictingIngredient))
            {
                return;
            }
        }
        ...
        warnings.Add(new IngredientConflictWarningDto
        {
            ...
            IngredientA = rule.PrimaryIngredientEntity?.Name ?? rule.PrimaryIngredient ?? string.Empty,
            IngredientB = rule.ConflictingIngredientEntity?.Name ?? rule.ConflictingIngredient ?? string.Empty,
